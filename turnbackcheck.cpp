...
QList<QgsWkbTypes::GeometryType> TurnbackCheck::compatibleGeometryTypes() const
{
    return factoryCompatibleGeometryTypes();
}

void TurnbackCheck::collectErrors(const QMap<QString, FeaturePool *> &featurePools, QList<CheckError *> &errors, QStringList &messages, QgsFeedback *feedback, const LayerFeatureIds &ids) const
{
    Q_UNUSED(messages)
    ...
    for (const CheckerUtils::LayerFeature &layerFeature : layerFeatures)
    {
        ...
        for (int iPart = 0, nParts = geom->partCount(); iPart < nParts; ++iPart)
        {
            for (int iRing = 0, nRings = geom->ringCount(iPart); iRing < nRings; ++iRing)
            {
                ...
                for (int iVert = !closed; iVert < nVerts - !closed - 1; ++iVert)
                {
                    ...
                    const QgsPoint &p4 = geom->vertexAt(QgsVertexId(iPart, iRing, (iVert + 2) % nVerts));
                    QgsVector v21, v23, v32, v34;
                    try
                    {
                        v21 = QgsVector(p1.x() - p2.x(), p1.y() - p2.y()).normalized();
                        v23 = QgsVector(p3.x() - p2.x(), p3.y() - p2.y()).normalized();
                        v32 = QgsVector(p2.x() - p3.x(), p2.y() - p3.y()).normalized();
                        v34 = QgsVector(p4.x() - p3.x(), p4.y() - p3.y()).normalized();
                    }
                    ...
                    double angle2 = std::acos(v32 * v34) / M_PI * 180.0;
                    if (angle < mMinAngle && angle2 < mMinAngle)
                    {
...
void TurnbackCheck::fixError(const QMap<QString, FeaturePool *> &featurePools, CheckError *error, int method, const QMap<QString, int> & /*mergeAttributeIndices*/, Changes &changes) const
{
...
QStringList TurnbackCheck::resolutionMethods() const
{
    static QStringList methods = QStringList() << QStringLiteral("删除第一个打折点") << QStringLiteral("无");
    return methods;
}

QString TurnbackCheck::id() const
{
    return factoryId();
}

QString TurnbackCheck::factoryDescription()
{
    return QStringLiteral("线内打折");
}

QString TurnbackCheck::description() const
{
    return factoryDescription();
}

Check::CheckType TurnbackCheck::checkType() const
{
    return factoryCheckType();
}

QList<QgsWkbTypes::GeometryType> TurnbackCheck::factoryCompatibleGeometryTypes()
{
    return {QgsWkbTypes::LineGeometry};
}

bool TurnbackCheck::factoryIsCompatible(QgsVectorLayer *layer)
{
    return factoryCompatibleGeometryTypes().contains(layer->geometryType());
}

QString TurnbackCheck::factoryId()
{
    return QStringLiteral("TurnbackCheck");
}

Check::CheckType TurnbackCheck::factoryCheckType()
{
