...
void LineLayerIntersectionCheck::collectErrors(const QMap<QString, FeaturePool *> &featurePools, QList<CheckError *> &errors, QStringList &messages, QgsFeedback *feedback, const LayerFeatureIds &ids) const
{
    Q_UNUSED(messages)
    ...
    for (const CheckerUtils::LayerFeature &layerFeatureA : layerFeaturesA)
    {
        ...
        for (int iPart = 0, nParts = geom->partCount(); iPart < nParts; ++iPart)
        {
            ...
            for (const CheckerUtils::LayerFeature &layerFeatureB : layerFeaturesB)
            {
                ...
                if (layerFeatureA.layer()->id() == layerFeatureB.layer()->id())
                {
                    continue;
                }
...
void LineLayerIntersectionCheck::fixError(const QMap<QString, FeaturePool *> &featurePools, CheckError *error, int method, const QMap<QString, int> & /*mergeAttributeIndices*/, Changes & /*changes*/) const
{
...
QStringList LineLayerIntersectionCheck::resolutionMethods() const
{
    static QStringList methods = QStringList() << QStringLiteral("无");
    return methods;
}

Check::CheckType LineLayerIntersectionCheck::factoryCheckType()
{
