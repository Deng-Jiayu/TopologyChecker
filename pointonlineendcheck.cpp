    ...
    else if (method == Snap)
    {
        ...
        for (const CheckerUtils::LayerFeature &checkFeature : checkFeatures)
        {
            ...
            for (int jPart = 0, mParts = testGeom->partCount(); jPart < mParts; ++jPart)
            {
                ...
                QgsPoint p;
                if (point->distance(testLine->startPoint()) < point->distance(testLine->endPoint()))
                    p = testLine->startPoint();
                else
                    p = testLine->endPoint();
...
{
    static QStringList methods = QStringList() << QStringLiteral("吸附至端点") << QStringLiteral("删除点要素") << QStringLiteral("无");
    return methods;
}
