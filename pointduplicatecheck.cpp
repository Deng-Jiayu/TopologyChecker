...
QString PointDuplicateCheckError::duplicatesString(const QMap<QString, FeaturePool *> &featurePools, const QMap<QString, QList<QgsFeatureId>> &duplicates)
{
    QStringList str;
...
void PointDuplicateCheck::collectErrors(const QMap<QString, FeaturePool *> &featurePools, QList<CheckError *> &errors, QStringList &messages, QgsFeedback *feedback, const LayerFeatureIds &ids) const
{
    ...
    {
        if (std::find(pointLayers.begin(), pointLayers.end(), layerFeatureA.layer()) == pointLayers.end())
            continue;
        ...
        {
            if (std::find(pointLayers.begin(), pointLayers.end(), layerFeatureB.layer()) == pointLayers.end())
                continue;
        ...
        {
            errors.append(new PointDuplicateCheckError(this, layerFeatureA, geomA.constGet()->centroid(), featurePools, duplicates));
        }
    }
}

void PointDuplicateCheck::fixError(const QMap<QString, FeaturePool *> &featurePools, CheckError *error, int method, const QMap<QString, int> & /*mergeAttributeIndices*/, Changes &changes) const
{
    ...
    else if (method == RemoveDuplicates)
    {
        ...
        PointDuplicateCheckError *duplicateError = static_cast<PointDuplicateCheckError *>(error);
...
QStringList PointDuplicateCheck::resolutionMethods() const
{
...
QString PointDuplicateCheck::factoryId()
{
    return QStringLiteral("PointDuplicateCheck");
}

Check::CheckType PointDuplicateCheck::factoryCheckType()
{
