 ...
 * ### Writing the check
 *
 * A new subclass of Check needs to be written and at least the following
 ...
 * - checkType(): One of Check.LayerCheck, Check.FeatureCheck,Check.FeatureNodeCheck
 ...
 * geometry check factory created per check type, but it's possible that multiple Check
 ...
 * abstract methods need to be implemented:
 *
 * - QgsGeometryCheckFactory::createGeometryCheck(): Needs to return a new subclassed Check object that has been written in the previous step.
 ...
 * - QgsGeometryCheckFactory::flags(): Returns additional flags for a geometry check. If unsure return Check.AvailableInValidation.
...
class Check
{
  ...
  public:
    ...
    struct Change
    {
       ...
       * Create a new Change
       */
      Change( Check::ChangeWhat _what, Check::ChangeType _type, QgsVertexId _vidx = QgsVertexId() )
        : what( _what )
        , type( _type )
        , vidx( _vidx )
      {}
     ...
     */
    typedef QMap<QString, QMap<QgsFeatureId, QList<Check::Change> > > Changes;

    /**
     * Create a new geometry check.
     */
    Check( const CheckContext *context, const QVariantMap &configuration );
     ...
     * \since QGIS 3.10
     */
    virtual void prepare( const CheckContext *context, const QVariantMap &configuration );
     ...
     */
    virtual Check::Flags flags() const;
     ...
     * \since QGIS 3.4
     */
    virtual void collectErrors( const QMap<QString, FeaturePool *> &featurePools, QList<CheckError *> &errors SIP_INOUT, QStringList &messages SIP_INOUT, QgsFeedback *feedback, const LayerFeatureIds &ids = Check::LayerFeatureIds() ) const = 0;
     ...
     * \since QGIS 3.4
     */
    virtual void fixError( const QMap<QString, FeaturePool *> &featurePools, CheckError *error, int method, const QMap<QString, int> &mergeAttributeIndices, Changes &changes SIP_INOUT ) const SIP_SKIP;
     ...
     * \since QGIS 3.12
     */
    virtual QList<CheckResolutionMethod> availableResolutionMethods() const;
     ...
     * \since QGIS 3.4
     */
    const CheckContext *context() const { return mContext; }
  ...
  protected:
     ...
     * \since QGIS 3.4
     */
    QMap<QString, QgsFeatureIds> allLayerFeatureIds( const QMap<QString, FeaturePool *> &featurePools ) const SIP_SKIP;
     ...
     * \since QGIS 3.4
     */
    void deleteFeatureGeometryPart( const QMap<QString, FeaturePool *> &featurePools, const QString &layerId, QgsFeature &feature, int partIdx, Changes &changes ) const SIP_SKIP;
     ...
     * \since QGIS 3.4
     */
    void deleteFeatureGeometryRing( const QMap<QString, FeaturePool *> &featurePools, const QString &layerId, QgsFeature &feature, int partIdx, int ringIdx, Changes &changes ) const SIP_SKIP;

    const CheckContext *mContext;
