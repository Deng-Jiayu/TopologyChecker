    ...
    else if (method == SnapToLine)
    {
        ...
        for (const CheckerUtils::LayerFeature &checkFeature : checkFeatures)
        {
            ...
            for (int jPart = 0, mParts = testGeom->partCount(); jPart < mParts; ++jPart)
            {
                ...
                QgsPoint p = CheckerUtils::getFootOfPerpendicular(*point, testLine, mContext->tolerance);
                if (init)
                {
                    ans->setX(p.x());ans->setY(p.y());
                    init = false;
                }

                if (point->distance(p) < point->distance(*ans)){
                    ans->setX(p.x());ans->setY(p.y());
                }
...
{
    static QStringList methods = QStringList() << QStringLiteral("吸附至线上") << QStringLiteral("删除点要素") << QStringLiteral("无");
    return methods;
}
